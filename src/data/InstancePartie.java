...
public class InstancePartie implements ListenerEvenementJeu {
	...
	private void phaseAction() {
		while (resteEncoreUnPersonnageAJouer()) {
			BoucleJoueurs:
			for (Joueur j : joueursTriesTour) {
				if (j.resteEncoreUnPersonnageAJouer()) {
					System.out
							.println("------------------------------------------------------------------------");
					System.out.println("Joueur : " + j.nom);
					Personnage p = null;
					while ((p = ihm.selectionnerPersonnage(j.getPersonnagesRestantsAJouer())) == null) {
						System.out.println("Vous devez sélectionner un personnage!");
					}
